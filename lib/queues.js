...
module.exports = {
  ...
  remove() {
    ...
    let queue = _this.queueStack.pop();
    let params = {
    ...
    return sqs.listQueues(params).promise()
      .then((data) => {
        ...
        for(var qu in data.QueueUrls) {
          var q = data.QueueUrls[qu];
          if(q.toString().endsWith(`/${queue.QueueName}`)) {
            return sqs.deleteQueue({
              ...
              .then(() => {
                _this.serverless.cli.log(`severless-sqs-queue Removed queue ${queue.QueueName}`);
                _this.remove();
              })
          }
        }
